...
import { SovereignSwarm, ExecuteTask, Signer } from "sovereign-swarm";
...
//       sdkConfig.pinataJwt = process.env.PINATA_JWT;
//     } else {
...
//             mcpServer: "https://fluidmcpserver.vercel.app/mcp",
//             timestamp: Date.now(),
...
//           feedbackFile.tag3
//         );
//         console.log("   Reviewer:", await signer.getAddress());
...
//        const data = await queryFeedbackFromSubgraph(sdk, "84532:16");
//        console.log({
//         data
//        });
...
//         const feedbackIndex =
//           Array.isArray(feedback.id) && feedback.id.length === 3
//             ? feedback.id[2]
//             : 0;
...
//           console.log(
//             "   Created At:",
//             new Date(retrievedFeedback.createdAt * 1000).toISOString()
//           );
...
const MCP_SERVER_URL = "https://fluidnewsagent.vercel.app/mcp";
const MCP_PROTOCOL_VERSION = "2025-11-20";

/**
 * Test MCP server connectivity
 */
async function testMCPConnectivity(url: string): Promise<boolean> {
  try {
    ...
    console.log(
      "❌ MCP server connectivity test failed:",
 ...
 */
async function queryAgentFromSubgraph(
  sdk: SovereignSwarm,
  agentId: string
): Promise<any> {
  try {
 ...
 */
async function searchAgentsInSubgraph(
  sdk: SovereignSwarm,
  filters: any = {}
): Promise<any[]> {
  try {
 ...
 */
async function queryFeedbackFromSubgraph(
  sdk: SovereignSwarm,
  agentId: string
): Promise<any> {
  try {
    ...
    feedbacks.forEach((fb, idx) => {
      console.log(
        `   ${idx + 1}. Score: ${fb.score}, Tags: ${fb.tags}, Reviewer: ${
          fb.reviewer
        }`
      );
    });
...
const main = async () => {
  try {
    ...
    if (!PRIVATE_KEY) {
      throw new Error(
        "PRIVATE_KEY not set in .env file - write operations require a signer"
      );
    }
    ...
    let feedbackSigner: ethers.Wallet | undefined;
    if (FEEDBACK_PRIVATE_KEY) {
      ...
      console.log(
        "   Feedback operations will use the same wallet as agent owner"
      );
    ...
    if (feedbackSigner) {
      ...
      console.log(
        "✅ Feedback Reviewer balance:",
        ethers.formatEther(feedbackBalance),
        "ETH"
      );
    ...
    const agentName = `CryptoPunkNews`;
    const agentDescription = "CryptoPunkNews is a Web3-focused media outlet delivering fast, community-driven coverage of crypto, NFTs, and blockchain culture. It highlights breaking market trends, major project updates, and insights from leading builders shaping decentralized technology. The platform blends news, analysis, and opinion pieces to help readers navigate emerging innovations across digital assets. With a pulse on on-chain activity and creative communities, CryptoPunkNews serves as a hub for staying ahead in the evolving crypto ecosystem.";
    const agentImage = "/doodles.jpg";
    ...
    console.log(
      "   IPFS CID:",
      registrationFile.agentURI?.replace("ipfs://", "")
    );
      ...
      try {
        ...
        if (ipfsResponse.ok) {
          ...
          if (mcpEndpoint) {
            ...
            console.log(
              `   Resources: ${mcpEndpoint.mcpResources?.length || 0}`
            );
    ...
    if (!feedbackSigner) {
      ...
      console.log(
        "   Add FEEDBACK_PRIVATE_KEY to .env to test feedback functionality"
      );
    } else {
      console.log("⚠️  This will cost gas!");
      ...
      try {
        ...
        try {
          const retrievedFeedback = await sdk.getFeedback(
            newAgentId,
            await feedbackSigner.getAddress(),
            feedbackIndex
          );
          console.log("✅ Feedback retrieved directly");
          console.log("   Score:", retrievedFeedback.score);
    ...
    if (!feedbackSigner) {
      ...
      console.log(
        `   2. Add to .env: FEEDBACK_PRIVATE_KEY="<new-private-key>"`
      );
      console.log(
        `   3. Get testnet ETH for the new wallet from https://sepoliafaucet.com/`
      );
