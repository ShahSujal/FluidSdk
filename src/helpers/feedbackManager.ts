...
import type { AgentId, Address, URI, Timestamp, IdemKey } from '../types/common.js';
import type { Web3Client } from './web3client.js';
import type { IPFSClient } from './ipfsClient.js';
import type { SubgraphClient } from './subgraphclient.js';
...
export class FeedbackManager {
  ...
  async searchFeedback(params: SearchFeedbackParams): Promise<Feedback[]> {
    ...
    if (formattedAgents || params.agents) {
      searchParams.agents = formattedAgents || params.agents;
    }
    if (params.reviewers) {
      searchParams.reviewers = params.reviewers;
    }
    if (params.tags) {
      searchParams.tags = params.tags;
    }
    if (params.capabilities) {
      searchParams.capabilities = params.capabilities;
    }
    if (params.skills) {
      searchParams.skills = params.skills;
    }
    if (params.tasks) {
      searchParams.tasks = params.tasks;
    }
    if (params.names) {
      searchParams.names = params.names;
    }
    if (params.minScore !== undefined) {
      searchParams.minScore = params.minScore;
    }
    if (params.maxScore !== undefined) {
      searchParams.maxScore = params.maxScore;
    }
    searchParams.includeRevoked = params.includeRevoked || false;
